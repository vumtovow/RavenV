...
import ravenv.data.Box;
import ravenv.event.EventManager;
import ravenv.events.PickEvent;
import ravenv.events.RaytraceEvent;
...
public abstract class MixinEntityRenderer {
    ...
    private float hurtCameraEffect(float float1) {
        if (RavenV.moduleManager == null) {
            ...
            NoHurtCam noHurtCam = (NoHurtCam) RavenV.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (RavenV.moduleManager != null) {
            GhostHand event = (GhostHand) RavenV.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private double v(Vec3 vec31, Vec3 vec32) {
        if (RavenV.moduleManager == null) {
            ...
            return RavenV.moduleManager.modules.get(ViewClip.class).isEnabled() ? (double) this.thirdPersonDistance : vec31.distanceTo(vec32);
        }
    }
    ...
    private Material x(Block block) {
        if (RavenV.moduleManager == null) {
            ...
            return RavenV.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && RavenV.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) RavenV.moduleManager.modules.get(AntiDebuff.class);
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && RavenV.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) RavenV.moduleManager.modules.get(AntiDebuff.class);
