...
import ravenv.events.PacketEvent;
import ravenv.events.TickEvent;
import ravenv.mixin.IAccessorC03PacketPlayer;
            ...
            if (event.getPacket() instanceof C03PacketPlayer) {
                ...
                switch (this.mode.getValue()) {
                    ...
                    case 1:
                        ...
                        if (RavenV.blinkManager.getBlinkingModule() != BlinkModules.NO_FALL) {
                            if (this.lastOnGround
                                ...
                                RavenV.blinkManager.setBlinkState(true, BlinkModules.NO_FALL);
                            }
                        } else if (!allowed) {
                            RavenV.blinkManager.setBlinkState(false, BlinkModules.NO_FALL);
                            ChatUtil.sendFormatted(String.format("%s%s: &cFailed player check!&r", RavenV.clientName, this.getName()));
                        } else if (PlayerUtil.checkInWater(mc.thePlayer.getEntityBoundingBox().expand(2.0, 0.0, 2.0))) {
                            RavenV.blinkManager.setBlinkState(false, BlinkModules.NO_FALL);
                            ChatUtil.sendFormatted(String.format("%s%s: &cFailed void check!&r", RavenV.clientName, this.getName()));
                        } else if (packet.isOnGround()) {
                            for (Packet<?> blinkedPacket : RavenV.blinkManager.blinkedPackets) {
