...
import ravenv.enums.FloatModules;
...
import ravenv.events.RightClickMouseEvent;
import ravenv.module.Module;
import ravenv.util.BlockUtil;
import ravenv.util.ItemUtil;
import ravenv.util.PlayerUtil;
...
public class NoSlow extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (this.isEnabled() && this.isFloatMode()) {
            ...
            if (this.lastSlot != item && PlayerUtil.isUsingItem()) {
                this.lastSlot = item;
                RavenV.floatManager.setFloatState(true, FloatModules.NO_SLOW);
            }
        } else {
            this.lastSlot = -1;
            RavenV.floatManager.setFloatState(false, FloatModules.NO_SLOW);
        }
    }
    ...
    public void onRightClick(RightClickMouseEvent event) {
        if (this.isEnabled()) {
            ...
            if (this.isFloatMode() && !RavenV.floatManager.isPredicted() && mc.thePlayer.onGround) {
